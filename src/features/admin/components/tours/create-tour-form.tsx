...
import { TourCategory } from "@/features/tours/types";
...
export const CreateTourForm = () => {
    ...
    const form = useForm({
        ...
        defaultValues: {
            name: "",
            description: "",
            category: TourCategory.WALKING_TOUR,
            basePrice: 0,
            currency: "USD",
            isFree: false,
            durationHours: 1,
    ...
    const mutation = useMutation({
        ...
        onError: (error: any) => {
            ...
            toast.error(Array.isArray(msg) ? msg[0] : msg || "Failed to create tour");
        }
    });
    ...
    const onSubmit = (data: CreateTourFormValues) => {
        ...
        console.log("🚀 Payload sending to API:", apiPayload);
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 pb-20">
                ...
                <div className="flex items-center justify-between sticky top-0 z-10 bg-white/80 backdrop-blur-md py-4 border-b border-gray-100 -mx-6 px-6">
                    <div className="flex items-center gap-4">
                        <Button type="button" variant="outline" size="icon" onClick={() => router.back()}><ArrowLeft size={18} /></Button>
                        <div>
                            <h1 className="text-2xl font-bold text-forest">Create New Journey</h1>
                            <p className="text-gray-500 text-sm">Publish a new tour experience.</p>
                        </div>
                    </div>
                    <Button type="submit" className="bg-terracotta hover:bg-terracotta/90 text-white" disabled={mutation.isPending}>
                        {mutation.isPending ? "Saving..." : <><Save size={16} className="mr-2" /> Save Tour</>}
                    </Button>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="col-span-full">
                                                ...
                                                <FormControl><Input placeholder="e.g. Hanoi Street Food Tour" {...field} value={field.value as string} /></FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl><Textarea placeholder="Describe the experience..." className="h-32" {...field} value={field.value as string} /></FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                />
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => append({ title: "", description: "", time: "", images: [] })}>
                                    <Plus size={16} className="mr-2" /> Add Step
                                </Button>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                {fields.map((field, index) => (
                                    <div key={field.id} className="relative flex gap-6 p-4 rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-md transition-all group">
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>Base Price ({form.getValues('currency')})</FormLabel>
                                            <FormControl><Input type="number" className="font-mono text-lg" {...field} value={field.value as number} /></FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl><Input placeholder="Opera House" {...field} value={field.value as string} /></FormControl>
                                        </FormItem>
                                    )}
                                />
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 bg-gray-50/50 mt-4">
                                            <div className="space-y-0.5"><FormLabel className="text-base">Free Tour</FormLabel></div>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 bg-gray-50/50">
                                            <div className="space-y-0.5"><FormLabel className="text-base">Pickup</FormLabel></div>
                                            <FormControl><Checkbox checked={field.value as boolean} onCheckedChange={field.onChange} /></FormControl>
                                        </FormItem>
                                    )}
                                />
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 bg-gray-50/50">
                                            <div className="space-y-0.5"><FormLabel className="text-base">Wheelchair</FormLabel></div>
                                            <FormControl><Checkbox checked={field.value as boolean} onCheckedChange={field.onChange} /></FormControl>
                                        </FormItem>
                                    )}
                                />
